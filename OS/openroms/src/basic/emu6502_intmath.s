...
;
; emu6502 port: minimal integer-only arithmetic support
;
; The upstream FRMEVL float support is incomplete (fetching float literals,
; pushing float values, and all the operators are stubs). For the 6502
; simulator port we provide a minimal integer-only implementation:
;
; - emu_fetch_float      - parses an unsigned integer literal (up to 65535)
;                          into FAC1 as a float; no decimals/exponents
; - emu_push_fac1_float  - pushes FAC1 onto the FRMEVL stack (5 bytes + type)
; - emu_pop_fac2         - pops a pushed float value into FAC2 (used by operators)
...
;
; emu_fetch_float - parse unsigned integer literal at TXTPTR into FAC1
...
;
; emu_pop_fac2 - pop value pushed by emu_push_fac1_float into FAC2
...
;
; emu_store_fac1_var - store FAC1 (5 bytes) into the variable at VARPNT
;
emu_store_fac1_var:
...
;
; emu_load_fac1_var - load FAC1 (5 bytes) from the variable at VARPNT
;
emu_load_fac1_var:
...
;
; emu_push_for_frame - push the 13 byte FOR frame from zero page locations:
