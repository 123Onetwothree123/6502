...
for i in range(0x54+len(patch), 0x7F):
    basic[i] = 0xEA
# 1.5 强制走默认检测路径（$C052 bne -> NOP）
basic[0x52:0x54] = bytes([0xEA, 0xEA])
...
w(0xA9); w(12); w(0x8D); w(0x00); w(0x3F)
...
seq[13] = table >> 8
code += seq
tbl = [0x0D, ord('6'), ord('*'), ord('7'), ord(' '), ord('T'), ord('N'), ord('I'), ord('R'), ord('P'), 0x0D, 0x0D]
code += tbl
