...
:       lda TXTPTR
        ...
        jsr rb_expr_find_return_rhs
        bcs :+
        jmp BASIC_SYNERR
:       lda rb_exec_out_type
        cmp #RB_OUT_STRING
        beq @string
        jsr rb_expr_eval_int_rhs
        jsr @restore
        lda LINNUM+1
        ldy LINNUM
        jsr BASIC_GIVAYF
...
:       clc
        ...
        jsr BASIC_PTRGET
        lda VARPNT
        cmp rb_exec_formal_lo
        bne @no
        lda VARPNT+1
        cmp rb_exec_formal_hi
        bne @no
        jsr rb_skip_spaces
        cmp #TOKEN_EQUAL
        bne @no
...
rb_expr_eval_int_rhs:
        jsr rb_expr_read_int_term
        lda LINNUM
        sta rb_saved_count_lo
        lda LINNUM+1
        sta rb_saved_count_hi
        ...
        clc
        lda LINNUM
        adc rb_saved_count_lo
        sta LINNUM
        lda LINNUM+1
        adc rb_saved_count_hi
        sta LINNUM+1
@done:
        rts

rb_expr_read_int_term:
        jsr rb_skip_spaces
        lda #0
        sta SUBFLG
        jsr BASIC_PTRGET
