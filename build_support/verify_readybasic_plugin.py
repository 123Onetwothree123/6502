...
def main() -> None:
    ...
    require(r"^BASIC_START\s*=\s*\$3001\b", asm, "BASIC_START must be $3001")
    ...
    if lowpack[0] != 0x1C00 or lowpack[1] > 0x23FF:
        fail(f"LOWPACK must fit in $1C00-$23FF, got ${lowpack[0]:04X}-${lowpack[1]:04X}")
    if hidden[0] != 0xA000 or hidden[1] > 0xA5FF:
        fail(f"HIDDEN helper must fit in $A000-$A5FF shadow, got ${hidden[0]:04X}-${hidden[1]:04X}")
    ...
    if bridge[0] != 0xC000 or bridge[1] >= 0xC600:
        fail(f"BRIDGE must stay below $C600, got ${bridge[0]:04X}-${bridge[1]:04X}")
    if regseed[2] > 0x0200:
